...
const CategoryPage = ({ categoryName, categorySlug }) => {
  ...
  const generateArticles = (pageNumber) => {
    return Array.from({ length: 6 }).map((_, i) => ({
      ...
      excerpt:
        "මෙය මෙම පුවත පිළිබඳ කෙටි හැඳින්වීමකි. වැඩිදුර තොරතුරු කියවීම සඳහා පහත බොත්තම ඔබන්න. අපගේ වෙබ් අඩවිය අලුත්ම තොරතුරු ඔබ වෙත ගෙන එයි...",
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-2/3 flex flex-col gap-6">
            ...
            {articles.map((article) => (
              <article
                key={article.id}
                className="group flex flex-col sm:flex-row gap-4 bg-white p-4 shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-lg cursor-pointer"
              >
                {/* Thumbnail */}
                ...
                <div className="sm:w-2/3 flex flex-col justify-center py-2">
                  ...
                  <div>
                    <Link
                      ...
                      className="inline-block bg-ecn-navy text-white text-xs font-medium px-4 py-2 hover:bg-ecn-dark-blue transition-colors rounded"
                    >
            ...
            <div className="mt-6 mb-8 text-center bg-white p-6 border border-gray-100 shadow-sm">
              <button
                ...
                disabled={isLoading}
                className={`font-bold py-3 px-10 rounded transition-all ${
                  isLoading
