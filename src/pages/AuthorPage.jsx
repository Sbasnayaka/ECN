...
import { getArticlesByAuthor } from '../api/articleService';
    ...
    <div className="w-full">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8 border-b-4 border-ecn-navy pb-4">
          <h1 className="text-3xl md:text-4xl font-black text-ecn-dark-blue">කර්තෘ : {authorName || "ප්‍රවෘත්ති අංශය"}</h1>
        </div>

        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3 flex flex-col gap-6">
            {articles.length === 0 && !loading && (
              <p className="text-gray-500">කර්තෘගේ පුවත් හමු නොවීය.</p>
            )}
              ...
              <article key={article.id} className="group flex flex-col sm:flex-row gap-4 bg-white p-4 shadow-sm border border-gray-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 rounded-lg cursor-pointer">
                <div className="sm:w-1/3 shrink-0 overflow-hidden rounded">
                  <Link to={`/article/${article.id}`}>
                    <img src={article.image_url || "https://via.placeholder.com/400x250/e0e0e0/888888"} alt={article.title} className="w-full h-48 sm:h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                  </Link>
                </div>
